...
Type
  ...
  Public
    ...
    Property OnPercent : LongInt Read FOnPercent Write FOnPercent;
...
Type
  ...
  Public
    Constructor Create(AInFile, AOutFile : TStream; ABufSize : LongWord);override;
    Procedure Compress; override;
    procedure Compress(Level:TCompressionlevel); reIntroduce;
    Class Function ZipID : Word; override;
  ...
  private
    function GetZ(AIndex : LongInt): TZipFileEntry;
    procedure SetZ(AIndex : LongInt; const AValue: TZipFileEntry);
  Public
    ...
    Property Entries[AIndex : LongInt] : TZipFileEntry Read GetZ Write SetZ; default;
  end;
  ...
  Public
    ...
    Property InMemSize : LongInt Read FInMemSize Write FInMemSize;
  ...
  private
    function GetFZ(AIndex : LongInt): TFullZipFileEntry;
    procedure SetFZ(AIndex : LongInt; const AValue: TFullZipFileEntry);
  Public
    Property FullEntries[AIndex : LongInt] : TFullZipFileEntry Read GetFZ Write SetFZ; default;
  end;
  ...
  Public
    ...
    Procedure UnZip(Item:TFullZipFileEntry);
    Procedure Clear;
    Procedure Examine;
  Public
    Property BufferSize : LongWord Read FBufSize Write SetBufSize;
...
procedure TDeflater.Compress(Level:TCompressionlevel);
Var
  Buf : PByte;
  I,Count,NewCount : LongInt;
  C : TCompressionStream;
...
begin
  ...
  FSize:=FInfile.Size;
  Try
    C:=TCompressionStream.Create(Level,FOutFile,True);
    Try
      Repeat
        Count:=FInFile.Read(Buf^,FBufferSize);
        For I:=0 to Count-1 do
...
Function TZipper.CheckEntries : LongInt;
...
Procedure TUnZipper.UnZip(Item:TFullZipFileEntry);
begin
  OpenInput();
  UnZipOneFile(Item);
  CloseInput();
...
function TZipFileEntries.GetZ(AIndex : LongInt): TZipFileEntry;
begin
  Result:=TZipFileEntry(Items[AIndex]);
end;

procedure TZipFileEntries.SetZ(AIndex : LongInt; const AValue: TZipFileEntry);
begin
  Items[AIndex]:=AValue;
end;
...
function TFullZipFileEntries.GetFZ(AIndex : LongInt): TFullZipFileEntry;
begin
  Result:=TFullZipFileEntry(Items[AIndex]);
end;

procedure TFullZipFileEntries.SetFZ(AIndex : LongInt;
