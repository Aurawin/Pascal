...
Version 2013 compiled for Windows 64
...
WINDOWS XP CAUTIONARY NOTE:
    ...
    networknode have access *ALL* nodes which host AuraDisks.  Since XP 
    does not support linking network drives to folders, AuraDisks cannot
    be maintained.

    While it is possible to mount 1 physical device to a folder, you will not
    be able to run Aurawin in a clustered environment.
...
MANIFEST (AuSCS.zip):
   ...
   install.bat - shell script for installation process.
   remove.bat	- shell script for removing SCS.

*********************************************************************
PREQUISITES:

1.) The necessary downloads listed in this section are available from our Downloads
    Application.  Or by visiting https://aurawin.com/downloads

2.) Aurawin install.bat script will create a folder "C:\Program Files\Aurawin\SCS".
    If you need to change that path edit the install.bat.

3.) Aurawin install script will create a folder "C:\Program Files\Aurawin\SCS\Logs for
...
4.) Aurawin requires a database managment system.  Presently, only three types
    ...
    Postgresql:
	Copy all dlls from C:\Program Files\PostgreSQL\9.2\bin to
	C:\Windows\System32
...
7.) At least one AuraDisk is required.
	AuraDisks automatically mount using NFS (unix). Under Windows, however,
	you must make sure you MAP network drives manually.  See AURADISKS below.
...
3.) from a command prompt type install
...
The installation folder (target C:\Program Files\Aurawin\SCS) contains an scs.ini file.
